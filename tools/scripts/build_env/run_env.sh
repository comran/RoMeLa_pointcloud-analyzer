...
BUILD_FLAGS="-t romela_build_env"
...
docker network create -d bridge romela_bridge > /dev/null 2>&1 || true
...
#       the docker environment, the root will need to be set to the
...
DOCKER_BUILD_CMD="set -x; \
  ...
  mkdir -p /tmp/home/romela; \
  usermod -d /tmp/home/romela romela; \
  usermod -u $(id -u) -g $(id -g) romela; \
  usermod -d /home/romela romela; \
  chown romela /home/romela; \
  chown romela /home/romela/.cache; \
  echo STARTED > /tmp/romela_init; \
  sudo -u romela bash -c \"bazel; \
  source /home/romela/.bashrc; \
...
docker run \
  ...
  --rm \
  --net=host \
  -v $ROOT_PATH:/home/romela/code_env \
  -e DISPLAY=$DISPLAY \
  -v /tmp/.X11-unix:/tmp/.X11-unix \
  --dns 8.8.8.8 \
  --name romela_build_env \
  romela_build_env \
  bash -c "$DOCKER_BUILD_CMD"

echo "Started romela_build_env docker image. Waiting for it to boot..."
...
./tools/scripts/build_env/exec.sh "while [ ! -f /tmp/romela_init ];do sleep 0.25;done"
